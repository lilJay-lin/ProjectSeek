var JCPlayer, dj, JCPjson, JCPotc, JCPrm = 0, JCPotnum, JCPtcnum;
var JCPdata = [];
var cmload = function () {
    ...
    function touch(event) {
        ...
        switch (event.type) {
            case'touchstart':
                sX.innerHTML = event.touches[0].clientX;
                sY.innerHTML = event.touches[0].clientY;
                break;
            case'touchend':
                eX.innerHTML = event.changedTouches[0].clientX;
                eY.innerHTML = event.changedTouches[0].clientY;
                var X1 = parseInt(eX.innerHTML);
                var X2 = parseInt(sX.innerHTML);
                var Y1 = parseInt(eY.innerHTML);
                var Y2 = parseInt(sY.innerHTML);
                var goX = X1 - X2;
                var goY = Y1 - Y2;
...
var openTC = function () {
    if (/iphone|android|nokia|sony|ericsson|mot|samsung|sgh|lg|philips|panasonic|alcatel|lenovo|cldc|midp|wap|mobile/i.test(navigator.userAgent.toLowerCase())) {
        document.write('<div id="txy" style="display:none;"><div id="sX" style="display:none;top:-22222px;left:-22222px"></div><div id="eX" style="display:none;top:-22322px;left:-22322px"></div><div id="sY" style="display:none;top:-22422px;left:-22422px"></div><div id="eY" style="display:none;top:-22522px;left:-22522px"></div></div>');
        window.addEventListener('load', cmload, false);
    }
};
openTC();
!function (a, b) {
...
;
(function ($) {
    $('<meta name="viewport" content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0,user-scalable=yes" /><meta name="apple-mobile-web-app-capable" content="yes" /><meta name="HandheldFriendly" content="true" /><meta http-equiv="Cache-control" content="no-cache"/>').appendTo('head');
    $.fn.womendi = function (c) {
        var a = {pop: '', sign: '5', t: '', gt: '', e: '', sc: ''};
        var c = $.extend(a, c);

        function wo(pop) {
            var w = $(window).width(), h = $(window).height();
            var t1 = $(window).scrollTop(), l1 = $(window).scrollLeft();
            var t = 0, l = 0;
            if (c.pop != '.mingigiao') {
                t = t1, l = l1
            }
            ;
            var pl, pt, pw = pop.outerWidth(true), ph = pop.outerHeight(true);
            switch (c.sign) {
                case'1':
                    pl = 0;
                    pt = 0;
            ...
            ;
            if (c.gt == 'animate') {
                pop.animate({'left': pl + 'px', 'top': pt + 'px'}, 500).dequeue()
            } else if (c.gt == 'show') {
                pop.css({left: pl + 'px', 'top': pt + 'px'}).hide().show(1000)
            } else if (c.gt == 'fadeIn') {
                pop.css({left: pl + 'px', 'top': pt + 'px'}).hide().fadeIn(1000)
            } else if (c.gt == 'slideDown') {
                pop.css({left: pl + 'px', 'top': pt + 'px'}).hide().slideDown()
            }
        }
    ...
    $.fn.epublog = function (g) {
        var a = {logdiv: '#logdiv', logtxt: '', lw: '', lh: '', e: 'slideDown', dm: 'body', type: '1'};
        var g = $.extend(a, g);
        if (g.type == '1') {
            $(g.logdiv).remove();
            $('<div id="' + (g.logdiv).substr(1) + '"><div>').appendTo(g.dm);
            $(g.logdiv).html(g.logtxt).css({
                width: g.lw + 'px',
                position: 'absolute',
                opacity: '0.8',
                background: '#f90',
            ...
            $(this).womendi({pop: g.logdiv, gt: g.e})
        } else if (g.type == '0') {
            $(g.logdiv).remove()
        }
    }
...
var Swiper = function (dmjson, dmurl, selector, params) {
    ...
    _this.JCP2num = function (iii) {
        header.imgNow = iii - 1;
        $('.swiper-wrapper').attr('n', iii);
        $('#nowhere').html(iii);
        $(params.dmNow).html(iii);
...
var otchange = function () {
    $(window).bind('orientationchange', function (e) {
        ...
        setTimeout(function () {
            JCPlayer = new Swiper(JCPdata.JCPjson, JCPdata.urls, JCPdata.div, {
                mode: mode,
                rm: '1',
                frame: JCPdata.frame,
                dmNum: JCPdata.dmNum,
...
$.fn.JCPlayer = function (b) {
    ...
    if (b.rm == '1') {
        ...
        JCPrm = 1;
        JCPotc = 1;
        JCPlayer = new Swiper(JCPjson, b.urls, b.div, {
            mode: b.mode,
            rm: b.rm,
            frame: b.frame,
            dmNum: b.dmNum,
        ...
        $.ajax({
            ...
            success: function (JCP) {
                var cm = 'The JCPlayer is running';
                JCPjson = JCP;
                JCPdata = {
                    JCPjson: JCPjson,
                    urls: b.urls,
                ...
                var ZK_JCPlayer = setInterval(function () {
                    if (cm) {
                        JCPlayer = new Swiper(JCP, b.urls, b.div, {
                            mode: b.mode,
                            rm: b.rm,
                            frame: b.frame,
                            dmNum: b.dmNum,
